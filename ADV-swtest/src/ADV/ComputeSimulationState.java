...
public class ComputeSimulationState {
	...
	private double alt_rate[]= {0.0,-3.2,-1.4,0.0}, watts_per_sec[]={0.0,2,1,0.0};
...
//				System.out.println(measurements.getVf()+" "+measurements.getVd());
			}
			...
			if (measurements.getshield_position().equals("D") & measurements.getshield_cmd().equals("D") & measurements.getVf() < 25.0 & measurements.getAltitude() < 50.0){
//				System.out.println(measurements.getshield_position()+" "+measurements.getshield_cmd());
				alerts.setISRZ(true);
			}
				...
				measurements.setMotor_state("Off");
					...
					if (!alerts.isPDMG() & (Math.abs(measurements.getCum_attitude()-measurements.getTerr_attitude())<=6))
						alerts.setPD(true);
							...
							rand=measurements.getRand();
							if ((rand<0.6) & (Math.abs(measurements.getCum_attitude()-measurements.getTerr_attitude())<=6))
								alerts.setPD(true);
							else if ((rand>=0.6) & (Math.abs(measurements.getCum_attitude()-measurements.getTerr_attitude())<=6)){ 
								alerts.setPND(true);
							}
							else
								alerts.setDC(true); 
